...
class DownloadDialog(ft.AlertDialog):
    ...
    def _build_content(self):
        ...
        self._page.services.append(self.folder_picker)
        self._page.update()
        
        async def on_pick_folder_click(e):
        ...
        def copy_magnet(_):
            self._page.clipboard.set(magnet_link)
            self._page.snack_bar = ft.SnackBar(content=ft.Text("Magnet link copied!"))
        ...
        info_panel = ft.Container(
            ...
            width=320,
            padding=15,
            bgcolor=ft.Colors.BLUE_GREY_900 if self._page.theme_mode == ft.ThemeMode.DARK else ft.Colors.BLUE_GREY_50,
            border_radius=10,
            expand=True,
        )
    ...
    def _on_save_torrent_click(self, e):
        ...
        try:
            ...
            # Check if it's a local file (FileTorrent)
                ...
                self._page.snack_bar = ft.SnackBar(content=ft.Text("Could not save: Source not available"))
                self._page.snack_bar.open = True
                self._page.update()
                return
            ...
            self._page.snack_bar = ft.SnackBar(content=ft.Text(f"Error: {ex}"))
            self._page.snack_bar.open = True
            self._page.update()
