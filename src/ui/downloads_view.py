...
class DownloadsView(ft.Container):
    def __init__(self, page, torrent_manager):
        debug_log("DownloadsView.__init__ started")
        super().__init__()
        self._page = page
        self.torrent_manager = torrent_manager
        
        # Add listener AFTER building view to avoid race conditions
        ...
        debug_log("DownloadsView calling _build_view")
        self.content = self._build_view()
        
        debug_log("DownloadsView adding listener")
        self.torrent_manager.add_listener(self._on_update)
        debug_log("DownloadsView.__init__ finished")

    def _build_view(self):
        debug_log("Inside _build_view")
        self.downloads_list = ft.ListView(expand=True, spacing=10, padding=10)
        debug_log("calling _refresh_list")
    ...
    def _show_add_magnet_dialog(self):
        ...
        def add_magnet(e):
            ...
            if not getattr(download, 'is_newly_added', False):
            ...
            self._page.overlay.append(loading_dlg)
                ...
                try:
                    ...
                    if found:
                        ...
                        def on_confirm(selected_files, download_path=None):
                            ...
                            # Move download to custom path if specified
                            # Note: For libtorrent-based downloads, the save path was already set
                            # This is handled during magnet metadata fetch
                        ...
                        download_dlg = DownloadDialog(self._page, download, self.torrent_manager, on_confirm, on_cancel=on_cancel, files=files)
                        
                        self._page.overlay.append(download_dlg)
                        ...
                        def on_retry(e):
                            ...
                            self._page.run_thread(poll_metadata)
                        ...
                        self._page.overlay.append(retry_dlg)
                        retry_dlg.open = True
                        self._page.update()
                except Exception as e:
    ...
    async def _pick_torrent_file(self, e):
        file_picker = ft.FilePicker()
        self._page.services.append(file_picker)
        self._page.update()
        files = await file_picker.pick_files(allow_multiple=False, allowed_extensions=["torrent"])
        ...
        if files:
            ...
            try:
                ...
                # Show dialog
                from ui.download_dialog import DownloadDialog
                dlg = DownloadDialog(self._page, torrent_obj, self.torrent_manager, on_confirm, files=files_list)
                self._page.overlay.append(dlg)
                dlg.open = True
                self._page.update()
    ...
    def _create_empty_state(self):
        """Create the 'No active downloads' placeholder"""
        return ft.Container(
            data="empty_state", # Use data attribute for easy identification
            content=ft.Column([
                ft.Icon(ft.Icons.DOWNLOAD_DONE, size=64, 
                       color=ft.Colors.GREY_400 if self._page.theme_mode == ft.ThemeMode.LIGHT else ft.Colors.GREY_600),
                ft.Text("No active downloads", 
                       color=ft.Colors.GREY_600 if self._page.theme_mode == ft.ThemeMode.LIGHT else ft.Colors.GREY_400,
                       weight=ft.FontWeight.W_500),
                ft.Text("Add a torrent to get started", 
                       color=ft.Colors.GREY_500 if self._page.theme_mode == ft.ThemeMode.LIGHT else ft.Colors.GREY_400, size=12)
            ], horizontal_alignment=ft.CrossAxisAlignment.CENTER),
            alignment=ft.Alignment.CENTER,
            padding=50
        )

    def _refresh_list(self, update_ui=True):
        if not self._page:
            return
        ...
        try:
            ...
            if not visible_downloads:
                ...
                # 1. Clean up our memory of controls that are no longer needed
                existing_ids = list(self.download_controls.keys())
                for old_id in existing_ids:
                    if old_id not in current_ids:
                        self.download_controls.pop(old_id)
                
                # 2. Build/Update cards for current downloads
                # visible_downloads is usually sorted by add time or status
                for item in visible_downloads:
                    if item.id not in self.download_controls:
                        # Create new card if we don't have one
                    ...
                    # Add to the new list (this maintains the manager's preferred order)
                    new_controls_list.append(self.download_controls[item.id]['card'])
                
                # 3. Definitively set the new controls list (this removes empty state automatically)
                self.downloads_list.controls = new_controls_list
    ...
    def _create_download_card(self, item):
        ...
        is_glass = False
        try:
            is_glass = self.torrent_manager.settings_manager.get('theme') == 'glass'
        except:
            pass
        ...
        card = ft.Card(
            ...
            bgcolor=ft.Colors.TRANSPARENT if is_glass else None
        )
    ...
    def _update_colors(self, item, progress_bar, status_text):
        ...
        if is_glass:
            # Glass Theme Colors (Neon/Pastel)
            if item.status == DownloadStatus.COMPLETED:
                progress_color = "#9ccfd8" # Cyan
            elif item.status == DownloadStatus.DOWNLOADING_METADATA:
                progress_color = "#f6c177" # Gold
            elif item.status == DownloadStatus.PAUSED:
                progress_color = "#6e6a86" # Muted
            elif item.status == DownloadStatus.ERROR or item.status == DownloadStatus.DELETED:
                progress_color = "#eb6f92" # Red/Pink
            elif item.status == DownloadStatus.SEEDING:
                progress_color = "#c4a7e7" # Purple
            elif item.status == DownloadStatus.STOPPED:
                progress_color = "#908caa" # Greyish
            elif item.status == DownloadStatus.QUEUED:
                progress_color = "#31748f" # Blue
            else: # Downloading
                progress_color = "#eb6f92" # Pink
            
            progress_bar.bgcolor = "#44415a" # Darker track
        else:
            # Default Colors
            ...
            progress_bar.bgcolor = ft.Colors.GREY_200 if self._page.theme_mode == ft.ThemeMode.LIGHT else ft.Colors.GREY_800
    ...
    def _show_files(self, download_id):
        ...
        try:
            ...
            if not item.has_metadata:
                ...
                for file_info in files:
                    ...
                    for part in path_parts[:-1]:
                        if part not in current_level['folders']:
                            current_level['folders'][part] = {'files': [], 'folders': {}}
                        current_level = current_level['folders'][part]
                    
                    # Add file to current level
                    current_level['files'].append(file_info)
                
                # Recursive function to build UI controls
                def build_level(level, depth=0):
                    controls = []
                    
                    # Add folders (sorted)
                    for folder_name in sorted(level['folders'].keys()):
                        folder_content = build_level(level['folders'][folder_name], depth + 1)
                        controls.append(
                            ft.ExpansionTile(
                                title=ft.Text(folder_name, size=13, weight=ft.FontWeight.BOLD),
                                leading=ft.Icon(ft.Icons.FOLDER, color=ft.Colors.BLUE, size=20),
                                controls=folder_content,
                                expanded=depth < 1, # Expand root folders by default
                                dense=True,
                                min_tile_height=40,
                            )
                        )
                    ...
                    # Add files (sorted)
                    for file_info in sorted(level['files'], key=lambda x: x['path'].replace('\\', '/').split('/')[-1]):
                        path = file_info['path']
                        ...
                        controls.append(
                            ft.ListTile(
                                leading=ft.Icon(ft.Icons.INSERT_DRIVE_FILE, size=18, color=text_color),
                ...
                # Build the tree UI
                dialog_content.controls.extend(build_level(tree))
            else:
    ...
    def _on_update(self):
        ...
        if has_changes or (current_time - self._last_update_time >= 0.8 and self.torrent_manager.downloads):
            with self._update_lock:
                if not self._is_refreshing:
                    self._last_update_time = current_time
                    self._page.run_thread(self._refresh_list)
    ...
    def _detect_changes(self):
        ...
        for download in self.torrent_manager.downloads:
            state_key = (
                ...
                round(download.progress, 4), # 0.01% precision
                download.download_rate, # Speed changes every second
    ...
    def handle_magnet_from_external(self, magnet_link):
        """Handle magnet link from external source (command line, browser protocol)"""
        # Create a dummy torrent object (same as in _show_add_magnet_dialog)
        class MagnetTorrent:
            def __init__(self, link):
                self.magnet = link
                self.name = "Fetching Metadata..."
        ...
        # Add torrent with visible=False to start fetching metadata
        download = self.torrent_manager.add_download(MagnetTorrent(magnet_link), visible=False)
        ...
        self._refresh_list()  # Force refresh to hide it
        
        # Show the same loading and file selection flow as in _show_add_magnet_dialog
        import threading
        import time
            ...
            try:
                ...
                if found:
                    ...
                    def on_confirm(selected_files, download_path=None):
                        ...
                        # Note: For libtorrent-based downloads, custom path handling
                        # would need to be done before resume
                        
                        self.torrent_manager.resume_download(download.id)
                        download.visible = True
                        self.torrent_manager._save_state()
                        self._refresh_list()
                        self._show_snack("Download started!")
    ...
    def add_torrent_file(self, file_path):
        """Handle torrent file from external source (command line, file association)"""
        try:
            if not os.path.exists(file_path):
                print(f"Torrent file not found: {file_path}")
                self._show_snack("❌ Torrent file not found")
                return
            
            print(f"Handling external torrent file: {file_path}")
            ...
            # Create dummy torrent object for dialog
            class FileTorrent:
                def __init__(self, path, name, size):
                    self.file_path = path
                    self.name = name
            ...
            from ui.download_dialog import DownloadDialog
            dlg = DownloadDialog(self._page, torrent_obj, self.torrent_manager, on_confirm, files=files)
            self._page.overlay.append(dlg)
            dlg.open = True
            self._page.update()
            
        except Exception as e:
            print(f"Error handling external torrent file: {e}")
            import traceback
            traceback.print_exc()
            self._show_snack(f"❌ Error: {str(e)}")
