...
class SettingsView(ft.Container):
    ...
    def _build_general_settings(self):
        ...
        # Close Confirmation
        def on_close_confirmation_change(e):
    ...
    def _build_download_settings(self):
        ...
        async def pick_download_folder(e):
            picker = ft.FilePicker()
            self._page.services.append(picker)
            self._page.update()
            path = await picker.get_directory_path(dialog_title="Select Download Folder")
            if path:
                download_folder_field.value = path
                self.settings_manager.set('download_folder', path)
                self.download_manager.base_path = path
                self._show_snack(f"Download folder set to: {path}")
        ...
        async def pick_temp_folder(e):
            picker = ft.FilePicker()
            self._page.services.append(picker)
            self._page.update()
            path = await picker.get_directory_path(dialog_title="Select Temporary Folder")
            if path:
                temp_folder_field.value = path
                self.settings_manager.set('temp_folder', path)
                self.download_manager.temp_path = path
                os.makedirs(path, exist_ok=True)
                self._show_snack(f"Temp folder set to: {path}")
    ...
    def _build_provider_settings(self):
        ...
        def show_add_provider_dialog(e):
            ...
            dialog = ft.AlertDialog(
                ...
                actions=[
                    ft.TextButton("Cancel", on_click=lambda e: setattr(dialog, 'open', False) or self._page.update()),
