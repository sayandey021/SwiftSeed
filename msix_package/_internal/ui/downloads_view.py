...
class DownloadsView(ft.Container):
            ...
            def poll_metadata():
                try:
                    ...
                    if found:
                        ...
                        download_dlg = DownloadDialog(self.page, download, self.torrent_manager, on_confirm, on_cancel=on_cancel, files=files)
    ...
    def _refresh_list(self, update_ui=True):
        ...
        try:
            ...
            if not visible_downloads:
                    ...
                    ft.Container(
                        content=ft.Column([
                            ...
                            ft.Text("Add a torrent to get started", 
                                   color=ft.Colors.GREY_500 if self.page.theme_mode == ft.ThemeMode.LIGHT else ft.Colors.GREY_400, size=12)
                ...
                # Remove empty state if present (old logic, replaced by direct assignment above)
                ...
                # Reverse to maintain order when inserting at top
                for item in reversed(visible_downloads):
                    if item.id not in self.download_controls:
                        ...
                        # Insert at the beginning (top) instead of appending to bottom
                        self.downloads_list.controls.insert(0, card)
                    else:
        ...
        if update_ui and self.page:
            try:
                ...
                # Silently ignore "Event loop is closed" errors - these happen during startup/shutdown
                if "Event loop is closed" not in str(e):
                    print(f"Error updating page: {e}")
            except Exception as e:
                # Only print other errors, not event loop issues
                if "Event loop is closed" not in str(e):
                    print(f"Error updating page: {e}")
    ...
    def _create_download_card(self, item):
        ...
        card = ft.Card(
            ...
            color=ft.Colors.TRANSPARENT if is_glass else None
        )
                    ...
                    for folder_name in sorted(level['folders'].keys()):
                        ...
                        controls.append(
                            ft.ExpansionTile(
                                ...
                                initially_expanded=depth < 1, # Expand root folders by default
                                dense=True,
                                min_tile_height=40,
                            )
                        )
